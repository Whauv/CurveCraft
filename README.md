...
CurveCraft is a production-style Python library for bond pricing, yield-curve construction, fixed-income risk
analytics, portfolio aggregation, visualizations, and a FastAPI surface for downstream integration.

## Motivation

This project is designed as a compact fixed-income analytics stack that is easy to inspect, test, and extend. It
covers the core building blocks a quantitative developer typically needs for vanilla rate products:
...
Static README chart generation uses `kaleido`, which is already included in the project dependencies.
...
CI is configured in [ci.yml](.github/workflows/ci.yml) for GitHub Actions.
